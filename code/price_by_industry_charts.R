...
# Only results with duration and logistic deterrence
dta <- dta[ duration == "wDur" ] 
dta <- dta[ deterrence == "threshold" ] 
nrow( dta ) == dta[ , length( unique( year ) ) * length( unique( nace2_2d_a64 ) ) ]   # Should be T
dta[ , c( "go2_a64", "duration", "deterrence" ) := NULL ]
...
# Time span in dataset (9999: average)
...
dta[ Effect == "spillover" , Effect := "Spillover" ]
